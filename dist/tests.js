(function () {
  ...
  */
  ...
  /* istanbul ignore next */
  const computeParams$1 = isNode$1 ? computeParamsNode$1 : computeParamsBrowser$1;
   ...
   * @return {string}
   */
  const getParam$1 = (name, defaultVal) => computeParams$1().get(name) || defaultVal;
  // export const getArgs = name => computeParams() && args

  const production$1 = getParam$1('production', '0') !== '0';
  ...
  const writeVarUint$1 = (encoder, num) => {
    while (num >= 0b10000000) {
      write$1(encoder, 0b10000000 | (bits7 & num));
      num >>>= 7;
    }
  ...
  const writeUint8Array$1 = (encoder, uint8Array) => {
    ...
    encoder.bufs.push(uint8Array);
    encoder.cbuf = createUint8ArrayFromLen$1(prevBufferLen);
    encoder.cpos = 0;
  };
  ...
  const readUint8Array$1 = (decoder, len) => {
    const view = createUint8ArrayViewFromArrayBuffer$1(decoder.arr.buffer, decoder.pos, len);
    decoder.pos += len;
    return view
  };
  ...
  const readSyncStep1 = (decoder, encoder, doc) =>
    writeSyncStep2(encoder, doc, readVarUint8Array$1(decoder));
  ...
  const testSnapshot = tc => {
    ...
    compare(state2Diff, [{insert: 'a'}, {insert: 'x', attributes: {ychange: { state: 'added' }}}, {insert: 'b', attributes: {ychange: { state: 'removed' }}}, { insert: 'cd' }]);
  };
